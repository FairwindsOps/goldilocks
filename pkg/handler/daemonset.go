...
// OnDaemonSetChanged is a handler that should be called when a DaemonSet chanages.
func OnDaemonSetChanged(daemonset *appsv1.DaemonSet, event utils.Event) {
	...
	if err != nil {
		klog.Errorf("Handler got error retrieving namespace object from event: %v. Breaking.", event)
		return
	}
